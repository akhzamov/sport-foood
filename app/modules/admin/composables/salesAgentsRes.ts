...
export const usePersonalSalesAgentsResponse = () => {
  ...
  async function getSalesAgentById(id: number) {
    const { $salesAgentsRep } = useNuxtApp();
    const personalStore = usePersonalStore();
    const profileStore = useProfileStore();
    try {
  ...
  async function editSalesAgentById(
    id: number,
    data: {
      name: string;
      contact: string;
  ...
  async function deleteSalesAgentById(id: number) {
